...
- [x] Show why the phone is currently muted, for example: muted by evening schedule or muted after leaving Home.
- [x] Show the next scheduled action, for example: next action today at 22:00, mute after 30 seconds.
- [x] Improve the permission screen with concrete diagnosis: what works, what blocks automation, and what is optional.
...
- [x] Warn when SSID is unavailable because location permission is missing, location is disabled, or Wi-Fi is off.
...
- [x] Prioritize the OLED theme as the first polished theme because it fits night-time utility use.
